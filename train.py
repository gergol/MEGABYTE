...
for i in tqdm.tqdm(range(NUM_BATCHES), mininterval=10.0, desc="training"):
    ...
    for __ in range(GRADIENT_ACCUMULATE_EVERY):
        loss = model(next(train_loader), return_loss=True, encoder_hidden_states=encoder_hidden_states)
        loss.backward()
    ...
    if i % VALIDATE_EVERY == 0:
        ...
        with torch.no_grad():
            ...
            loss = model(next(val_loader), return_loss=True, encoder_hidden_states=encoder_hidden_states)
